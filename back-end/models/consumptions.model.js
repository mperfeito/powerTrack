...
import { DataTypes, Op, fn, col, literal } from 'sequelize';
import sequelize from '../config/connect.js'; // seu sequelize já configurado
import moment from 'moment';

// Definir modelo Sequelize para consumption_readings
const ConsumptionReading = sequelize.define('consumption_reading', {
  ...
  id_house: { type: DataTypes.INTEGER, allowNull: false },
  consumption_value: { type: DataTypes.FLOAT, allowNull: false },
...
// Definir modelo Sequelize para houses (apenas id e city para usar em queries)
const House = sequelize.define('house', {
  id_house: { type: DataTypes.INTEGER, primaryKey: true },
...
// Definir modelo Sequelize para appliances
const Appliance = sequelize.define('appliance', {
  id_house: { type: DataTypes.INTEGER, primaryKey: false },
  type: { type: DataTypes.STRING },
  nominal_power_watts: { type: DataTypes.FLOAT },
  operating_hours: { type: DataTypes.FLOAT }
}, {
  tableName: 'appliances',
  timestamps: false
});

// Exportar os métodos como objeto padrão
export default {
  async insertConsumptions(houseId, value) {
    try {
      await ConsumptionReading.create({
        id_house: houseId,
  ...
  async comparePeriod(houseId, period) {
    try {
      ...
      const avg = await ConsumptionReading.findOne({
        attributes: [[fn('AVG', col('consumption_value')), 'avg_consumption']],
        where: {
          id_house: houseId,
          reading_date: {
  ...
  async getAverageConsumption(houseId, period, date) {
    const avg = await ConsumptionReading.findOne({
      attributes: [[fn('AVG', col('consumption_value')), 'avg']],
      where: {
        id_house: houseId,
        [Op.and]: [
          sequelize.where(fn('MONTH', col('reading_date')), date.month() + 1),
          sequelize.where(fn('YEAR', col('reading_date')), date.year())
        ]
      },
      raw: true
    });
  ...
  async compareWithSimilarHouses(houseId) {
    try {
      ...
      const currentHouse = await ConsumptionReading.findOne({
        attributes: [[fn('AVG', col('consumption_value')), 'avg']],
        where: { id_house: houseId },
        raw: true
      });

      const currentAvg = Number(currentHouse?.avg) || 0;

      const similarHouses = await House.findAll({
        attributes: [
          'id_house',
          [fn('AVG', col('consumption_readings.consumption_value')), 'avg']
        ],
        include: [{
          model: ConsumptionReading,
          as: 'consumption_readings',
          attributes: []
        }],
        where: {
          city: houseData.city,
          id_house: { [Op.ne]: houseId }
        },
        group: ['house.id_house'],
        raw: true
      });
      ...
      return {
        selectedHouse: {
          id: houseId,
          address: houseData?.address || "Endereço não encontrado",
        },
        comparedHouses,
        comparison: difference,
      };
  ...
  async compareDevices(houseId) {
    try {
      const totalResult = await ConsumptionReading.findOne({
        attributes: [[fn('IFNULL', fn('SUM', col('consumption_value')), 0), 'total']],
        where: { id_house: houseId },
        raw: true
      });

      const totalConsumption = Number(totalResult?.total) || 0;

      const devices = await Appliance.findAll({
        attributes: [
          'type',
          [fn('SUM', literal('nominal_power_watts * operating_hours / 1000')), 'estimated_kwh']
        ],
        where: { id_house: houseId },
        group: ['type'],
        order: [[literal('estimated_kwh'), 'DESC']],
        limit: 1,
        raw: true
      });
