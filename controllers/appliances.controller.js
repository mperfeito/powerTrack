...
// Get all appliances for a specific house
export const getAllAppliances = (req, res) => {
    ...
    appliancesModel.getAppliancesByHouseId(id_house, (err, appliances) => {
        ...
        if (!appliances || appliances.length === 0) {
            return res.status(404).json({ message: 'No appliances found for this house' });
        }
...
// Get a specific appliance by its ID and house ID
export const getApplianceById = (req, res) => {
    const { id_house: houseId, id_appliance: applianceId } = req.params;
...
export const createAppliance = (req, res) => {
    ...
    if (!type || !state || !avg_operating_hours || !nominal_power_watts) {
        return res.status(400).json({ errorMessage: 'All fields (type, state, avg_operating_hours, nominal_power_watts) are required' });
    }

    appliancesModel.addAppliance(id_house, type, state, avg_operating_hours, nominal_power_watts, (err, appliance) => {
        if (err) {
...
export const deleteAppliance = (req, res) => {
    const { id_house, id_appliance } = req.params;
    ...
    appliancesModel.deleteAppliance(id_house, id_appliance, (err, result) => {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ errorMessage: 'The requested resource could not be found on the server' });
        }
