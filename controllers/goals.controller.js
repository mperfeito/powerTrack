    ...
    if (goal.period_type === 'monthly_reduction') {
        ...
        try {
            const currentConsumption = await new Promise((resolve, reject) => {
                goalsModel.getTotalConsumptionByPeriod(id_house, currentMonthStart.toISOString().slice(0, 10), currentMonthEnd.toISOString().slice(0, 10),
                    (err, total) => {
                        if (err) reject(err);
                        resolve(total || 0);
                    }
            ...
            const referenceConsumption = await new Promise((resolve, reject) => {
                goalsModel.getTotalConsumptionByPeriod(id_house, previousMonthStart.toISOString().slice(0, 10), previousMonthEnd.toISOString().slice(0, 10),
                    (err, total) => {
                        if (err) reject(err);
                        resolve(total || 0);
                    }
    ...
    if (goal.period_type === 'peak_hour') {
        ...
        try {
            ...
            const peakConsumption = await new Promise((resolve, reject) => {
                goalsModel.getTotalConsumptionByDateTimeRange(id_house, peakHourStart, peakHourEnd, (err, total) => {
...
export const getAllGoals = async (req, res) => {
    ...
    try {
        ...
        const goalsWithStatus = await Promise.all(goals.map(goal => calculateGoalStatus(id_house, goal)));
...
export const getGoalById = async (req, res) => {
    const { id_house, id_goal } = req.params;
...
export const addGoal = async (req, res) => {
    ...
    if (!period_type || !target_value) {
        return res.status(400).json({ errorMessage: 'Period type and target value are required' });
    }
    const newGoal = {id_house, period_type, target_value, start_date: start_date || null, end_date: end_date || null
    };
    try {
        ...
        res.status(201).json({ message: 'Resource successfully created', goal: insertedGoal });
...
export const deleteGoal = (req, res) => {
    ...
    goalsModel.deleteGoal(id_house, id_goal, (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).json({ errorMessage: 'Failed to process request' });
        }
